...
fac_CR = 7.0 # face contraction ratio
...
thrust_sweep = np.linspace(2000,4000,num=3)
bt_sweep = np.linspace(6.0,20.0,num=4)
MR_sweep = [1.6,2.0]
PC_sweep = [21.0]
i = 0  # index
out = pd.DataFrame()
alt_targ = 5000  # m
alt_margin = 1000  # m - buffer safety factor
...
for thrust in thrust_sweep:
    for MR in MR_sweep:
        for PC in PC_sweep:
            ...
            """
            print(f'Running Blowdown Sim for PC: {PC} [BarA], MR: {MR} [-], Thrust: {eng.thrust} [N]')
            blowdown = gas_dyn_utils.blowdown_sensitivity_study(vol_sweep, [P_p_BOL], T_p_LOAD, P_p_EOL,
                                                                P_o_tank, q_ox, CF_ox,
                                                                P_f_tank, q_f, CF_fuel,
                                                                1.0,
                                                                PRESSGASS,
            ...
            for burntime in bt_sweep:
                ...
                V_p_BOL = np.interp(burntime,blowdown['time'],blowdown.index)
                m_p_i = np.interp(burntime,blowdown['time'],blowdown['mass_i'])
                m_p_f = np.interp(burntime,blowdown['time'],blowdown['mass_f'])
                ...
                print(f'Case {i+1} >> {thrust} [N], {burntime} [s], {PC} bar, {MR} [-] -- Max Altitude = {np.round(alt_max,2)}') # 1 based indexing like a n00b
                out.at[i, 't'] = thrust
                ...
                out.at[i, 'V_p_req'] = V_p_BOL*1000  # m**3 -> L
                i = i+1
            print('\n')
...
for MR in MR_sweep:
    for PC in PC_sweep:
        ...
        vp_req = np.round([np.interp((alt_targ+alt_margin),case_filt[case_filt['t'] == thrust]['h'],case_filt[case_filt['t'] == thrust]['V_p_req']) for thrust in thrust_sweep],2)
        ax3.plot(thrust_sweep,vp_req,label=f'PC: {PC} [BarA], MR: {MR} [-]')
        ax4.plot(thrust_sweep,bt_req,label=f'PC: {PC} [BarA], MR: {MR} [-]')
...
ax3.set_xlabel('Thrust [N]')
ax4.set_xlabel('Thrust [N]')

# titles
ax3.set_title(f'Thrust vs. Required Pressurant  -  alt: {alt_targ+alt_margin} [m]')
ax4.set_title(f'Thrust vs. Required Burn Time  -  alt: {alt_targ+alt_margin} [m]')
...
fig1.suptitle(f'Burn Time vs Thrust Trade - MR={MR}\n'
              ...
              f'Bottle Pressure: {np.round(P_p_BOL,2)} [BarA] ({np.round(P_p_BOL * BAR2PSI)} [psia])'
              f'Collapse Factors: (fuel: {CF_fuel}, ox: {CF_ox})\n'
              f'Tanks: {tank_MATL}\n'
